...
- The BOTD cron operation is currently set to operate every minute rather than once a day so that it is possible to actually see the /botd endpoint returning different and random values for the BOTD. The correct CRON expression is present in the scheduler but commented out. 

### Explaination of separated DB schema
A proper production-ready and normalised DB schema for this task is present in the `20250926212502-create-normalized-bean-schema-up` migration. I started writing out the API/DB migrations with the idea of adding it into a single table given the simplicity of the JSON schema. However I realised after getting the DB/API/migrations/etc set up with the seed.json file structure mirrored into the bean table that the task will also have been asking to write a schema that is more normalize and production-ready. I hope you're able to excuse that the API uses the more simple schema of having all the JSON data in a single table, and that the normalized schema is more of a representation of what I would do if the task was more involved than an API responsible for returning the data for 15 beans. 
